...
## Step 1: Building and Deploying a Docker Image With AWS CodePipeline

### Original Code from https://github.com/aws-samples/aws-cdk-examples/tree/master/typescript/codepipeline-build-deploy
...
>
> This example is built on Construct Libraries marked "Stable" and does not have any infrastructure prerequisites to build. You do need to connect code commit credentials to the user account to update the pipeline though.
...
- Steps to push to remote CodeCommit Repo to trigger pipeline (CodeCommit Credentials must be present)
...
- The simple containerized application contains no pages.
...
- Create a react frontend and nginx server.
- Create a backend.
- Try to add cognito user auth for front-end either cognito or amplify w. cognito.
...
- Set up a [Route 53 domain](https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/routing-to-elb-load-balancer.html) to route traffic to an Application Load Balancer (ALB).# ecs-mern-project
