...
export class EcomStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // Update the CodePipeline source action to point to the CodeCommit repository
    const sourceOutput = new pipeline.Artifact("SourceArtifact");

    const sourceAction = new pipelineactions.CodeCommitSourceAction({
      actionName: "AppCodeCommit",
      repository: codeRepo,
      output: sourceOutput,
    ...
    new cdk.CfnOutput(this, "ManualInstructions", {
      value: `
      1. Add the CodeCommit repo as a remote to your local repo:
        -> git remote add codecommit ${codeRepo.repositoryCloneUrlHttp}
      2. Fetch the contents from the CodeCommit repo:
        -> git fetch codecommit
      3. Checkout the main branch from CodeCommit:
        -> git checkout codecommit/main  
      4. Override the files by copying your local files:
        -> rm -rf ./*
        -> cp -r ../'app copy'/* .
      5. Commit the changes:
        -> git add .
        -> git commit -m "Override files" 
      6. Checkout main locally:
        -> git checkout -b main 
      7. Pushing the changes to CodeCommit:
        -> git push -u codecommit main
      `,
      description: "Manual Instructions for CodeCommit Repository",
    });
  }
}
